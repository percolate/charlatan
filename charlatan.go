...
func (g *Generator) generate() {
	...
	g.Printf("// Code generated by \"charlatan %s\"; DO NOT EDIT.\n", strings.Join(os.Args[1:], " "))
	g.Printf("\n")
	...
	for _, i := range interfacedecs {
		...
		tmpl, err := template.New("charlatantemplate").Parse(charlatanTemplate)
		if err != nil {
			panic(err)
		}

		for _, m := range i.Methods {
			err = tmpl.ExecuteTemplate(&g.buf, "INVOCATION", m)
			if err != nil {
				panic(err)
			}
		}

		err = tmpl.ExecuteTemplate(&g.buf, "FAKE", i)
		if err != nil {
			panic(err)
		}

		for _, m := range i.Methods {
			err = tmpl.ExecuteTemplate(&g.buf, "METHOD", m)
			if err != nil {
				panic(err)
			}
		}
...
// Might be a good idea to clean this up and/or move it into a separate file
const charlatanTemplate = `
